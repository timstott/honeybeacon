...
export const projectHasFaults = (projectConfig) => {
  return new Promise((resolve, reject) => {
    const { authToken, lastFaultReportedAt, projectId, query } = projectConfig;
    const queryString = querystring.stringify({
      auth_token: authToken,
      q: query,
    });
    const url = `${HONEYBADGER_ENDPOINT}/${projectId}/faults?${queryString}&occurred_after=${lastFaultReportedAt}`;
